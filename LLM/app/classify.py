...
def _profiles() -> List[Profile]:
    return [
        ...
        Profile(
            match=lambda p: p.startswith("/etc/systemd/") or p.startswith("/lib/systemd/system/") or p.startswith("/usr/lib/systemd/system/"),
            category="systemd 서비스 / 자동 실행 영속성 메커니즘",
            severity="High",
            mitre=["T1543.002", "T1565.001"],
            role="부팅·서비스 재시작 시 자동 실행되는 systemd 유닛 정의입니다. 변조 시 정상 서비스처럼 위장한 백도어 또는 명령 실행 경로가 지속적으로 복구될 수 있습니다.",
            scenarios=[
                "정상 점검 서비스처럼 보이는 유닛을 등록해 재부팅 후에도 공격자 스크립트가 자동 실행되도록 만드는 시도입니다.",
                "기존 서비스의 ExecStart·Environment 값을 변조해 정상 프로세스 실행 흐름에 공격자 명령을 끼워 넣는 시도입니다.",
            ],
            followups=[
                "systemctl cat, systemctl status, journalctl 출력으로 유닛 원문·활성화 시각·실행 이력을 확인합니다.",
                "동일 호스트의 cron, sudoers, SSH 설정 변경과 시간순으로 대조해 영속성 확보 체인을 재구성합니다.",
            ],
        ),
        Profile(
...
MITRE_GLOSSARY: Dict[str, Tuple[str, str]] = {
    ...
    "T1059.006": ("Python 실행", "Python 인터프리터로 임의 스크립트 또는 명령을 실행하는 행위입니다."),
    ...
    "T1074": ("데이터 스테이징", "수집·실행할 파일을 임시 위치에 모아 다음 단계 공격을 준비하는 행위입니다."),
    "T1074.001": ("로컬 데이터 스테이징", "로컬 임시 디렉터리 등에 도구·페이로드를 모아 실행 또는 반출을 준비하는 행위입니다."),
