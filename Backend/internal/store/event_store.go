...
func (s *MySQLEventStore) SaveEvent(ctx context.Context, p internal.FileEventPayload) error {
	query := `
		INSERT INTO file_events (agent_id, event_type, file_path, file_name, file_permission, detected_by, pid, blocked, occurred_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`

	_, err := s.db.ExecContext(ctx, query,
		p.AgentID, p.EventType, p.FilePath, p.FileName,
		p.FilePermission, p.DetectedBy, p.Pid, p.Blocked,
...
func (s *MySQLEventStore) QueryEvents(ctx context.Context, f internal.EventFilter) ([]internal.FileEvent, error) {
	query := `SELECT id, agent_id, event_type, file_path, file_name, file_permission, detected_by, pid, blocked, occurred_at, received_at FROM file_events WHERE 1=1`
	args := []interface{}{}
	...
	for rows.Next() {
		...
		err := rows.Scan(&e.ID, &e.AgentID, &e.EventType, &e.FilePath, &e.FileName, &e.FilePermission, &e.DetectedBy, &e.Pid, &e.Blocked, &e.OccurredAt, &e.ReceivedAt)
		if err != nil {
			return nil, err
		}
